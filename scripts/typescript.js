...
function print(filepath /* : string */) /* : void */ {
  ...
  const typescript = `export as namespace WebIDL2;
...
function printIDLInterface(idlInterface /* : IDLInterface */) /* : string */ {
  const constructor = idlInterface.extAttrs.find(extAttr => {
    return extAttr.name === 'Constructor';
  });
  let out = `export ${constructor ? 'class' : 'interface'} ${idlInterface.name}${idlInterface.inheritance ? ` extends ${idlInterface.inheritance}` : ''} {\n`;
  if (constructor) {
    out += `  constructor(${printIDLArguments(constructor.arguments || [])});\n`
  }
