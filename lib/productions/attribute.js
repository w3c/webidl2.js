...
export class Attribute extends Base {
  ...
  static parse(tokeniser, { special, noInherit = false, readonly = false } = {}) {
    ...
    ret.idlType = type_with_extended_attributes(tokeniser, "attribute-type") || tokeniser.error("No type in attribute");
    switch (ret.idlType.generic) {
      case "sequence":
      case "record": tokeniser.error(`Attributes cannot accept ${ret.idlType.generic} types`);
    }
    tokens.name = tokeniser.consume("identifier", "required") || tokeniser.error("No name in attribute");
    tokens.termination = tokeniser.consume(";") || tokeniser.error("Unterminated attribute");
    return ret;
  }
