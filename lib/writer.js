...
(function () {
    ...
    var write = function (ast, opt) {
        ...
        var type = function (it) {
            ...
            if (it.generic === "record") return "record<" + it.idlType.map(type).join(", ") + ">";
            var ret = "";
        ...
        var iterable = function(it) {
          return "iterable<" + (it.idlType instanceof Array
            ? it.idlType.map(type).join(", ")
            : type(it.idlType)
          ) + ">;";
        };
        var legacyiterable = function(it) {
          return "legacyiterable<" + (it.idlType instanceof Array
            ? it.idlType.map(type).join(", ")
            : type(it.idlType)
          ) + ">;";
        };
