 ...
 * @property {"normal" | "weak"} severity
 *
 * @param {string} message error message
 * @param {"Syntax" | "Validation"} type error type
 * @param {WebIDL2ErrorOptions} [options]
 */
function error(source, position, current, message, type, { severity = "normal" } = {}) {
  /**
   * @param {number} count
   */
  ...
  const withSeverity = severity !== "normal" ? `, with severity "${severity}"` : "";
  const grammaticalContext = current ? `, ${contextType} \`${current.partial ? "partial " : ""}${current.type} ${current.name}\`` : "";
  const context = `${type} error at line ${line}${inSourceName}${grammaticalContext}${withSeverity}:\n${sourceContext}`;
  return {
