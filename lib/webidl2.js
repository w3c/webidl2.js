...
function parseByTokens(source) {
  ...
  class Constructor extends Definition {
    static parse() {
      const base = consume("constructor");
      if (!base) {
        return;
      }
      const tokens = { base };
      tokens.open = consume("(") || error("No argument list in constructor");
      const args = argument_list();
      tokens.close = consume(")") || error("Unterminated constructor");
      tokens.termination = consume(";") || error("No semicolon after constructor");
