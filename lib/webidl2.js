...
(() => {
  ...
  function parse(tokens) {
    ...
    function argument_list() {
      const ret = [];
      const arg = Argument.parse();
      if (!arg) return ret;
      ret.push(arg);
      while (arg.separator) {
        const nxt = Argument.parse() || error("Trailing comma in arguments list");
        ret.push(nxt);
    ...
    function extended_attrs() {
      ...
      const first = SimpleExtendedAttribute.parse() || error("Extended attribute with not content");
      eas.items.push(first);
      while (first.separator) {
        const attr = SimpleExtendedAttribute.parse() || error("Trailing comma in extended attribute");
        eas.items.push(attr);
    ...
    class Interface {
      static parse(base, { callback, partial } = {}) {
        const tokens = { callback, partial, base };
        const ret = new Interface({ tokens });
        tokens.name = consume(ID) || error("No name for interface");
        current = ret;
        if (!partial) {
          sanitize_name(ret.name, "interface");
          ret.inheritance = Inheritance.parse() || null;
        }
        tokens.open = consume("{") || error("Bodyless interface");
        ret.members = [];
        while (true) {
          tokens.close = consume("}");
          if (tokens.close) {
            tokens.termination = consume(";") || error("Missing semicolon after interface");
            return ret;
          }
          const ea = extended_attrs();
          const mem = Constant.parse() ||
            static_member() ||
            stringifier() ||
            IterableLike.parse() ||
            Attribute.parse() ||
    ...
    class Mixin {
      static parse(base, { partial } = {}) {
        ...
        const ret = new Mixin({ tokens });
        tokens.mixin = consume("mixin");
        if (!tokens.mixin) {
          return;
        }
        tokens.name = consume(ID) || error("No name for interface mixin");
        current = ret;
        if (!partial) {
          sanitize_name(ret.name, "interface mixin");
        }
        ret.members = [];
        tokens.open = consume("{") || error("Bodyless interface mixin");
        while (true) {
          tokens.close = consume("}");
          if (tokens.close) {
            tokens.termination = consume(";") || error("Missing semicolon after interface mixin");
            return ret;
          }
          const ea = extended_attrs();
          const mem = Constant.parse() ||
    ...
    class Namespace {
      static parse({ partial } = {}) {
        ...
        const ret = new Namespace({ tokens });
        tokens.base = consume("namespace");
        if (!tokens.base) {
          return;
        }
        tokens.name = consume(ID) || error("No name for namespace");
        current = ret;
        if (!partial) {
          sanitize_name(ret.name, "namespace");
        }
        ret.members = [];
        tokens.open = consume("{") || error("Bodyless namespace");
        while (true) {
          tokens.close = consume("}");
          if (tokens.close) {
            tokens.termination = consume(";") || error("Missing semicolon after namespace");
            return ret;
          }
          const ea = extended_attrs();
          const mem = Attribute.parse({ noInherit: true, readonly: true }) ||
            Operation.parse({ regular: true }) ||
            error("Unknown member");
          mem.extAttrs = ea;
          ret.members.push(mem);
    ...
    class Dictionary {
      static parse({ partial } = {}) {
        const tokens = { partial };
        const ret = new Dictionary({ tokens });
        tokens.base = consume("dictionary");
        if (!tokens.base) {
          return;
        }
        tokens.name = consume(ID) || error("No name for dictionary");
        current = ret;
        if (!partial) {
          sanitize_name(ret.name, "dictionary");
          ret.inheritance = Inheritance.parse() || null;
        }
        tokens.open = consume("{") || error("Bodyless dictionary");
        ret.members = [];
        while (true) {
          tokens.close = consume("}");
          if (tokens.close) {
            tokens.termination = consume(";") || error("Missing semicolon after dictionary");
            return ret;
          }
    ...
    class Enum {
      static parse() {
        ...
        const values = [];
        current = new Enum({ tokens, values });
        ...
        while (true) {
          tokens.close = consume("}");
          if (tokens.close) {
            if (!values.length) error("No value in enum");
