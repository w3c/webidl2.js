...
(() => {
  ...
  function parse(tokens) {
    ...
    function generic_type(typeName) {
      ...
      const ret = new Type({ tokens: { base } });
      const open = consume("<") || error(`No opening bracket after ${base.type}`);
      ret.generic = { value: base.value, trivia: { open: open.trivia } };
      switch (base.type) {
        case "Promise":
      ...
      const close = consume(">") || error(`Missing closing bracket after ${base.type}`);
      ret.generic.trivia.close = close.trivia;
      return ret;
    }

    function single_type(typeName) {
      let ret = generic_type(typeName) || primitive_type();
      if (!ret) {
