(function() {
  var tokenise = function(str) {
    ...
    while (str.length > 0) {
      ...
      for (var i = 0, n = types.length; i < n; i++) {
        var type = types[i];
        str = str.replace(re[type], function(tok, offset) {
          tokens.push({ type: type, value: tok, offset: initialLength - str.length + offset });
          matched = true;
          return "";
        });
        if (matched) break;
  ...
  var parse = function(tokens, str, opt) {
    var line = 1;
    tokens = tokens.slice();
    ...
    var union_type = function() {
      ...
      ret.origin = `(${ret.idlType.map(type => type.origin.trim()).join(" or ")})${ret.nullable ? "?" : ""}`;

      return ret;
    };

    var type = function() {
      const beforeOffset = tokens[0].offset;
      const result = single_type() || union_type();
      const afterOffset = tokens[0].offset;
      if (result && !result.origin) {
        result.origin = str.slice(beforeOffset, afterOffset).trim();
      }
    ...
    var type_with_extended_attributes = function() {
      ...
      if (ret && !ret.origin) {
        ret.origin = str.slice(beforeOffset, afterOffset).trim();
      }
