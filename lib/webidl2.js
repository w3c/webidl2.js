(function() {
  ...
  var parse = function(tokens, opt) {
    ...
    var sanitize_name = function(name) {
      if (names.has(name)) {
        error(`Cannot have duplicated name "${name}"`);
      }
      names.add(name);
      return name;
    }
    ...
    var callback = function(store) {
      ...
      ret = { type: "callback", name: sanitize_name(name.value) };
      all_ws();
    ...
    var interface_ = function(isPartial, store) {
      ...
      var mems = [],
        ret = {
          type: "interface",
          name: isPartial ? name.value : sanitize_name(name.value),
          partial: false,
          members: mems
        };
    ...
    var typedef = function(store) {
      ...
      ret.name = sanitize_name(name.value);
      all_ws();
