...
(function () {
    ...
    var parse = function (tokens) {
        ...
        var implements_ = function () {
            all_ws();
            var target = consume_known("identifier");
            if (!target) return;
            var w = all_ws();
            if (consume_known("identifier", "implements")) {
                var ret = {
                    type:   "implements"
                ,   target: target.value
                };
                all_ws();
                var imp = consume_known("identifier") || error("Incomplete implements statement");
                ret["implements"] = imp.value;
                all_ws();
                consume_known("other", ";") || error("No terminating ; for implements statement");
                return ret;
            }
            else {
                // rollback
