...
(function() {
  function tokenise(str) {
    ...
    const extAttrsNotAllowedOnDict = new Set([
      "NoInterfaceObject"
    ])
  ...
  function parse(tokens, opt) {
    ...
    function dictionary(isPartial, store) {
      ...
      all_ws();
      const notAllowed = Array.isArray(store) && store
        .filter(({ name }) => extAttrsNotAllowedOnDict.has(name))
        .map(({ name }) => name)
        .join(", ");
      if (notAllowed) {
        error(`Found extended attribute(s) not allowed on dictionary: ${notAllowed}.`);
      }
