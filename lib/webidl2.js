  ...
  function parse(tokens) {
    ...
    class Argument {
      constructor() {
        ...
        this.idlType = type_with_extended_attributes("argument-type");
        if (!this.idlType) {
          return halt(start_position);
        }
        if (!optional) {
          this.tokens.variadic = consume("...");
        }
        this.tokens.name = consume(ID, ...argumentNameKeywords);
        if (!this.tokens.name) {
          return halt(start_position);
        }
        this.default = optional ? tryParse(Default) : null;
        this.tokens.separator = consume(",");
      }
    ...
    function argument_list() {
      const ret = [];
      const arg = tryParse(Argument);
      if (!arg) return ret;
      ret.push(arg);
      while (arg.separator) {
        const nxt = tryParse(Argument) || error("Trailing comma in arguments list");
        ret.push(nxt);
    ...
    class SimpleExtendedAttribute {
      constructor() {
        ...
        if (assign) {
          this.tokens.secondaryName = consume(ID, FLOAT, INT, STR);
        }
        this.tokens.open = consume("(");
        if (this.tokens.open) {
          const list = (assign && !this.tokens.secondaryName) ?
            // [Exposed=(Window,Worker)]
            identifiers() :
            // [NamedConstructor=Audio(DOMString src)] or [Constructor(DOMString str)]
            argument_list();
          Object.defineProperty(this, "list", { value: list });
          this.tokens.close = consume(")") || error("Unexpected token in extended attribute argument list");
        }
        if (assign && !this.rhs) error("No right hand side to extended attribute assignment");
        this.tokens.separator = consume(",");
      }
    ...
    function extended_attrs() {
      ...
      const first = tryParse(SimpleExtendedAttribute) || error("Extended attribute with not content");
      eas.items.push(first);
      while (first.separator) {
        const attr = tryParse(SimpleExtendedAttribute) || error("Trailing comma in extended attribute");
        eas.items.push(attr);
    ...
    class Constant {
      constructor() {
        ...
        this.idlType = Object.assign({ type: "const-type" }, EMPTY_IDLTYPE, typ);
        type_suffix(this.idlType);
        ...
        Object.defineProperties(this, {
          tokens: {
            value: { base, name, assign, termination }
          },
          primitive: {
            value: primitive
          }
      ...
      get trivia() {
        return {
          ...
          assign: this.tokens.assign.trivia,
          value: this.primitive.trivia,
    ...
    class Inheritance {
      ...
      get trivia() {
        return {
          colon: this.tokens.colon.trivia,
          name: this.tokens.name.trivia
        };
      }
    ...
    class CallbackFunction {
      constructor(base) {
        ...
        current = this;
        sanitize_name(name.value, "callback");
        this.tokens.assign = consume("=") || error("No assignment in callback");
        this.idlType = return_type() || error("Missing return type");
        this.tokens.open = consume("(") || error("No arguments in callback");
        this.arguments = argument_list();
        this.tokens.close = consume(")") || error("Unterminated callback");
        this.tokens.termination = consume(";") || error("Unterminated callback");
      }
    ...
    function callback() {
      ...
      if (tok) {
        return new Interface(tok, { typeName: "callback interface", callback });
      }
      return new CallbackFunction(callback);
    }
    ...
    class Interface {
      constructor(base, { callback = null, partial = null } = {}) {
        ...
        if (!partial) {
          ...
          this.inheritance = tryParse(Inheritance) || null;
        }
    ...
    class Field {
      constructor() {
        ...
        this.default = tryParse(Default) || null;
