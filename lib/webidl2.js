...
(() => {
  ...
  function parse(tokens) {
    ...
    function attribute({ special, noInherit = false, readonly = false } = {}) {
      const start_position = consume_position;
      const ret = {
        type: "attribute",
        special: null,
      ...
      if (!noInherit) {
        ret.special = untype_token(special) || untyped_consume("inherit") || null;
      }
    ...
    function static_member() {
      ...
      if (!special) return;
      const member = attribute({ special }) ||
